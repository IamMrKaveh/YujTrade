...
class TelegramBotHandler:
    def __init__(
        ...
        self.admin_chat_id = (
            str(admin_chat_id_raw).strip() if admin_chat_id_raw else None
        )

        if not self.admin_chat_id:
            logger.error(
                "CRITICAL: Admin Chat ID is not configured in environment or config!"
            )
        else:
    ...
    async def quick_analyze(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        self.background_tasks.create_task(
            analysis_task(), name=f"QuickAnalyze-{query.message.chat_id}"
        )
    ...
    async def full_analyze(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        self.background_tasks.create_task(
            analysis_task(), name=f"FullAnalyze-{query.message.chat_id}"
        )
