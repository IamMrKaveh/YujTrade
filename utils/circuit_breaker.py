...
from ..common.exceptions import APIRateLimitError, NetworkError
...
class CircuitBreaker:
    def __init__(
        ...
        self.failure_exceptions = failure_exceptions or {APIRateLimitError, NetworkError}
        ...
        async with self._lock:
            if self.state == CircuitState.OPEN:
                if ((time.time() - self.last_failure_time) > self.timeout):
                    ...
                    raise Exception(f"Circuit breaker is OPEN. Last failure at {self.last_failure_time}")
