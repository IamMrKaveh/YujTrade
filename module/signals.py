...
class SignalGenerator:
    def __init__(self, market_data_provider: MarketDataProvider, news_fetcher: Optional[NewsFetcher], market_indices_fetcher: Optional[MarketIndicesFetcher], model_manager: ModelManager, multi_tf_analyzer: MultiTimeframeAnalyzer, config: Dict, binance_fetcher: Optional[BinanceFetcher], messari_fetcher: Optional[MessariFetcher], analysis_engine):
        self.market_data_provider = market_data_provider
    ...
    async def generate_signals(self, data: pd.DataFrame, symbol: str, timeframe: str) -> Optional[TradingSignal]:
        return await self.analysis_engine.run_full_analysis(symbol, timeframe, data)