    ...
    async def wait_if_needed(self, endpoint: str):
        async with self._lock:
            ...
            if len(reqs) >= self.max_requests:
                ...
                now = time.time()
                self.requests[endpoint] = [t for t in reqs if now - t < self.time_window]
...
def calculate_fibonacci_levels(data: pd.DataFrame, lookback: int = 100) -> Dict[str, float]:
...
def calculate_pivot_points(data: pd.DataFrame) -> Dict[str, float]:
    ...
    return {
        ...
        'woody_pivot': woody_pivot, 'woody_r1': woody_r1, 'woody_s1': woody_s1,
        'cam_r4': camarilla_r4, 'cam_r3': camarilla_r3, 'cam_r2': camarilla_r2, 'cam_r1': camarilla_r1,
        'cam_s1': camarilla_s1, 'cam_s2': camarilla_s2, 'cam_s3': camarilla_s3, 'cam_s4': camarilla_s4,
    }


def calculate_dynamic_levels(data: pd.DataFrame, signal_type, market_context, order_book=None) -> Dict[str, Any]:
...
def calculate_risk_reward_ratio(entry_price: float, stop_loss: float, take_profit: float, signal_type) -> float:
...
def validate_signal_timing(data: pd.DataFrame, signal_type, 
                          min_candles_from_extreme: int = 3) -> Tuple[bool, str]:
    ...
    if signal_type == 'buy':
        ...
        candles_since_low = len(recent_data) - recent_data.index.get_loc(lowest_index) - 1
        ...
        candles_since_high = len(recent_data) - recent_data.index.get_loc(highest_index) - 1
...
def detect_market_regime(data: pd.DataFrame, lookback: int = 50) -> Dict[str, Any]:
    ...
    try:
        adx = talib.ADX(recent_data['high'].to_numpy(), 
...
def calculate_max_drawdown(data: pd.DataFrame) -> Tuple[float, int]:
    ...
    if isinstance(data.index, pd.DatetimeIndex):
        ...
        duration = int(data.index.get_loc(dd_end_idx) - data.index.get_loc(dd_start_idx))
...
class IndicatorNormalizer:
    ...
    def normalize_indicator(self, indicator_result, indicator_name: str):
        if not hasattr(indicator_result, 'value') or indicator_result.value is None or pd.isna(indicator_result.value):
            return indicator_result
        
        expected_min, expected_max = self.expected_ranges.get(indicator_name.lower(), (-1, 1))
        
        if expected_max == float('inf') and expected_min == -float('inf'):
            return indicator_result
        elif expected_max == float('inf'):
            if indicator_result.value > expected_min:
                normalized_value = min((indicator_result.value - expected_min) / 10 + 0.5, 1.0)
            else:
                normalized_value = -1.0
        elif expected_min == -float('inf'):
            if indicator_result.value < expected_max:
                normalized_value = max((indicator_result.value - expected_max) / 10 - 0.5, -1.0)
            else:
                normalized_value = 1.0
        else:
            if expected_max > expected_min:
                normalized_value = 2 * (indicator_result.value - expected_min) / (expected_max - expected_min) - 1
